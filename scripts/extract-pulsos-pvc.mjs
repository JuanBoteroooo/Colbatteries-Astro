/**
 * extract-pulsos-pvc.mjs — Pulsos PVC catalog extractor
 *
 * PyMuPDF full-page pixmap → tmp PNG → 600×600 pipeline
 *
 * Usage: node scripts/extract-pulsos-pvc.mjs
 */
...
const OUT_IMG   = path.join(ROOT, 'public/images/productos/pulso-pvc');
const OUT_JSON  = path.join(ROOT, 'src/data/pulsos-pvc.json');
const TMP_DIR   = path.join(tmpdir(), '_pulsos_pvc_pix');
...
PDF     = 'public/catalogos/pulso-pvc.pdf'
DPI     = 150
TMP_DIR = sys.argv[1]

MEDIDA_RE = re.compile(r'\b(\d{1,2}MM)\b', re.IGNORECASE)
COLOR_RE  = re.compile(r'\b(NEGRO|CAFE|BLANCO|ROJO|AZUL|VERDE|GRIS|BEIGE|MIEL|NATURAL|TRANSPARENTE|CRISTAL|NARANJA|AMARILLO|MORADO|ROSADO|MULTICOLOR|SURTIDO|BRONCE|DORADO|PLATA)\b', re.IGNORECASE)
...
for i in range(1, len(doc)):
    ...
    # Deduplicate medidas preserving order
    raw_medidas = MEDIDA_RE.findall(text.upper())
    seen = set(); uniq = []
    for m in raw_medidas:
        if m not in seen: seen.add(m); uniq.append(m.lower())
    medida = ', '.join(uniq)
...
const tmpPy = path.join(tmpdir(), '_extract_pulsos_pvc.py');
...
for (const { modelo, descripcion, medida, color, page, tmp_path } of pages) {
    ...
    jsonOut.push({
        ...
        medida,
        color,
        img: `/catalogo-pages/pulso-pvc/page-${padded}.jpg`,
    });
...
console.log(`📄 JSON → src/data/pulsos-pvc.json`);
