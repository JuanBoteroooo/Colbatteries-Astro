/**
 * extract-pulsos-metalicos.mjs — Pulsos Metálicos catalog extractor
 *
 * PyMuPDF full-page pixmap → tmp PNG → 600×600 pipeline
 *
 * Usage: node scripts/extract-pulsos-metalicos.mjs
 */
...
const OUT_IMG   = path.join(ROOT, 'public/images/productos/pulso-metalico');
const OUT_JSON  = path.join(ROOT, 'src/data/pulsos-metalicos.json');
const TMP_DIR   = path.join(tmpdir(), '_pulsos_metalicos_pix');
...
PDF     = 'public/catalogos/pulso-metalico.pdf'
DPI     = 150
TMP_DIR = sys.argv[1]

MEDIDA_RE = re.compile(r'\b(\d{1,2}MM)\b', re.IGNORECASE)
COLOR_RE  = re.compile(r'\b(PLATEADO|DORADO|ORO\s*ROSA|PAVONADO|BICOLOR|NEGRO|ROSADO|CHAMPAGNE|BRONCE)\b', re.IGNORECASE)
MAT_RE    = re.compile(r'\b(ACERO|METAL|MALLA)\b', re.IGNORECASE)
...
for i in range(1, len(doc)):
    ...
    medidas = MEDIDA_RE.findall(text.upper())
    medida  = ', '.join(dict.fromkeys(m.lower() for m in medidas)) if medidas else ''

    colors = COLOR_RE.findall(text.upper())
    color  = colors[0].capitalize() if colors else ''
    # Normalize "Oro Rosa" spacing
    color = re.sub(r'ORO\s*ROSA', 'Oro Rosa', color, flags=re.IGNORECASE)

    mats  = MAT_RE.findall(text.upper())
    material = mats[0].capitalize() if mats else ''
...
const tmpPy = path.join(tmpdir(), '_extract_pulsos_metalicos.py');
...
const CROP_OPTS = { topScanTo: 0.40, bottomScanFrom: 0.90 };
...
for (const { modelo, descripcion, medida, color, material, page, tmp_path } of pages) {
    let safe = modelo
        .replace(/\//g, '_')
        .replace(/ /g, '-')
        .replace(/#/g, 'num')
    ...
    jsonOut.push({
        ...
        color,
        material,
        img: `/catalogo-pages/pulso-metalico/page-${padded}.jpg`,
    });
...
console.log(`📄 JSON → src/data/pulsos-metalicos.json`);
