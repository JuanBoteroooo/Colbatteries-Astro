/**
 * extract-pulsos-silicona.mjs — Pulsos de Silicona catalog extractor
 *
 * PyMuPDF full-page pixmap → tmp PNG → 500×170 pipeline
 *
 * Usage: node scripts/extract-pulsos-silicona.mjs
 */
...
const OUT_IMG   = path.join(ROOT, 'public/images/productos/pulso-silicona');
const OUT_JSON  = path.join(ROOT, 'src/data/pulsos-silicona.json');
const TMP_DIR   = path.join(tmpdir(), '_pulsos_silicona_pix');
...
PDF     = 'public/catalogos/pulso-silicona.pdf'
DPI     = 150
TMP_DIR = sys.argv[1]
...
const tmpPy = path.join(tmpdir(), '_extract_pulsos_silicona.py');
...
const CROP_OPTS = { topScanTo: 0.10, bottomScanFrom: 0.60 };
...
for (const { modelo, descripcion, medida, page, tmp_path } of pages) {
    ...
    jsonOut.push({
        ...
        descripcion,
        medida,
        img: `/catalogo-pages/pulso-silicona/page-${padded}.jpg`,
    });
...
console.log(`📄 JSON → src/data/pulsos-silicona.json`);
